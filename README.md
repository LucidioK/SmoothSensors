...
| `CompassCalibration.h` | Drives the motor-powered, gyro-closed-loop calibration spin behind the `calibrate_compass` command: spins the robot in place via `RobotMotors`, integrating the IMU's `rz` (fed in by `sketch.ino`) to track rotation until it passes 380° (a 20° overshoot margin) or an 8s timeout elapses, then — if it actually rotated at least 300° — hands the spin off to `SmoothCompass::finishCalibration()` to compute the offset/scale correction. Pure state/logic like `SmoothCompass`/`SmoothDistance`/`SmoothMovement`: no Monitor or display code of its own, just getters (`isActive()`, `hasSucceededOnce()`, etc.) for `sketch.ino` to report status from. |
