...
| Magnetometer (LIS3MDL) | Not used for v1 turning (see §4 — turns are dead-reckoned from the IMU's Z-axis rotation rate instead). Would add absolute-heading correction for long-run drift. | **Wired and reporting, calibration in progress.** `sketch/SmoothCompass.h` is wired into `sketch.ino`/the RPC surface and reads real hardware (no boot-hang — the `recoverBus()` guard holds). Raw headings are distorted by a hard-iron bias (~3x Earth's field) from onboard magnetic material, confirmed empirically (readings only sweep ~40° across all four compass points instead of 360°). Fix: motor-driven, gyro-closed calibration spin — see §4b. |
...
- **v1 command vocabulary:**
  ...
  - `"robot calibrate"` → triggers the compass calibration spin (MCU-side `move("calibrate_compass")` command) → see §4b.
...
- **Trigger:** voice command `"robot calibrate"` (added to the Vosk grammar in `python/VoiceCommands.py` alongside the existing move commands), which calls the MCU-side `move("calibrate_compass")` RPC (same RPC surface as `go_ahead`/`turn_right`/`turn_left`/`stop`, see §3).
...
2. Whether v1 ever needs absolute heading (magnetometer) beyond relative turn-by-degrees — deferred to phase 2; revisit if drift over longer autonomous runs (e.g. "find and follow") proves to be a problem in practice. `SmoothCompass.h` is now wired into `sketch.ino` and reads real hardware; raw headings are usable but distorted by hard-iron bias from onboard magnetic material (motors/battery/wiring) — see §4b for the motor-driven calibration fix (in progress). Still open after that: a mounting-rotation/declination trim constant, and whether motor-current drift while actively driving needs separate handling
