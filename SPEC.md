...
| Magnetometer (LIS3MDL) | Not used for v1 turning (see §4 — turns are dead-reckoned from the IMU's Z-axis rotation rate instead). Would add absolute-heading correction for long-run drift. | **Phase 2 work resumed in software.** `sketch/SmoothCompass.h` now exists — same ring-buffer smoothing pattern as `SmoothDistance`/`SmoothMovement`, plus a dedicated I2C bus-recovery guard (`recoverBus()`) run before any sensor transaction, specifically to fail safely instead of re-hanging the board the way the earlier bare-wiring attempt did. Compiled and code-reviewed. **Not yet wired into `sketch.ino`/the RPC surface, and not yet tested against physical hardware** — the sensor itself still isn't wired to the board. |
...
2. Whether v1 ever needs absolute heading (magnetometer) beyond relative turn-by-degrees — deferred to phase 2; revisit if drift over longer autonomous runs (e.g. "find and follow") proves to be a problem in practice. Software groundwork (`SmoothCompass.h`) now exists, but the sensor still isn't wired to the board and nothing calls the new class yet — hardware bring-up and RPC/sketch.ino wiring remain open
