...
| Power | **Two separate rails**: one for Uno Q logic (delivered via the USB-C hub's power passthrough), one dedicated battery pack feeding the Modulino Motors' screw terminals directly (5–24V). Avoids brownouts from motor current sagging the logic supply. Sketch-side mitigations planned for the eventual motor battery regardless of which is chosen (see §3): (1) set the MAX22211's ITRIP current limit to ~1.5A per channel (matching the motors' rated stall current) to cap heat/mechanical stress during stall; (2) if the battery voltage exceeds the motors' 3–6V rating, cap the maximum PWM duty cycle sent to the driver so effective running voltage stays ≈6V. | **Motor rail battery: REJECTED, not yet replaced.** [WAVYPO 9V 1300mAh Li-ion](https://www.amazon.com/dp/B0BC8P2TS6) was proposed, but a customer review of this exact product reports an internal boost converter that caps steady-state output around ~100mA and sags below 6V above ~120mA draw (not an official datasheet figure, but specific and consistent with how these "smoke-alarm-style" 9V blocks are built) — far below what even normal (non-stall) driving needs (~300mA+ combined for two TT motors), let alone stall current (~1.1–1.5A per motor). Do not wire this battery to the motors. Two standard alternatives to choose between next session: a 2S LiPo pack (~7.4V, needs the PWM cap above, needs LiPo-aware charging) or a 6V NiMH pack (5×AA, matches motor voltage directly, no PWM cap needed, bulkier). |
...
1. **Motor battery.** Motors are chosen (see §2, TT gear motor kit). The proposed WAVYPO 9V battery was rejected as unable to sustain motor current (see §2 Power row) — need to pick between a 2S LiPo pack or a 6V NiMH pack (or a different specific product, to be verified the same way), then set the final ITRIP (~1.5A/channel) and, if needed, PWM duty-cycle cap values.
